...
> 🔑 **SECURE cloud 를 기본으로 쓰세요 (launcher 기본값).** emacs-jupyter 는 `ssh -L` 터널이
> 필수인데, SECURE cloud 는 전용 public IP 로 컨테이너에 직접 닿아 가장 안정적입니다.
>
> ⚠️ **(미검증 — 추후 확인 필요)** 예전에 COMMUNITY cloud 에서 `ssh -L` 이 실패한 적이 있어
> "exposed-TCP 포트가 컨테이너 sshd 로 라우팅되지 않는다"고 적어 두었으나, 이는 **확실히
> 검증되지 않았습니다.** 그때의 실패는 사실 **SSH 키 passphrase**(키는 맞아도 비대화형 접속이
> 거부됨 — 아래 `ssh-add` 주의 참고) 때문이었을 가능성이 큽니다. 이 문제는 cloud 종류와 무관하게
> 발생합니다. COMMUNITY 를 못 쓴다고 단정하기 전에 **passphrase 부터 배제**하고 다시 확인하세요.
>
> 단, SECURE 는 GPU 재고가 더 적습니다 — 3090 이 없으면 A4500(20GB)/A100(80GB) 등 Ampere 호환
> GPU 로 대체하세요. (`--gpu`/DC 가용성은 launcher 로 조회)
